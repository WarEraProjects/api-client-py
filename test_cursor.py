...
async def inspect_cursor():
    ...
    try:
        page2 = await warera.transaction.get_paginated(user_id="690d6b03becd7485dbb33b05", limit=2, cursor=iso_cursor)
    ...
    js_date = (datetime.now(timezone.utc) - timedelta(days=60)).strftime("%a %b %d %Y %H:%M:%S GMT+0000 (Coordinated Universal Time)")
    js_cursor = f"{js_date}|000000000000000000000000"
    try:
        page3 = await warera.transaction.get_paginated(user_id="690d6b03becd7485dbb33b05", limit=2, cursor=js_cursor)
