...
def resource_page_slug(name: str) -> str:
...
def run() -> None:
    ...
    # Remove previously generated resource pages so deleted resources don't linger
    for existing in os.listdir(REFERENCE_DIR):
        if existing.startswith("resource-") and existing.endswith(".md"):
            os.remove(os.path.join(REFERENCE_DIR, existing))

    client = warera.WareraClient()
    resources = []
    for prop in dir(client):
        if prop.startswith("_") or prop in [
            "batch",
    ...
    for name, res in sorted(resources, key=lambda x: x[0]):
        page_slug = resource_page_slug(name)
        file_path = os.path.join(REFERENCE_DIR, f"{page_slug}.md")

        print(f"Generating {page_slug}.md...")
        md = generate_resource_page(name, res)
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(md)

        api_ref_lines.append(
            f"- [`client.{name}`]({page_slug}.md) - {res.__class__.__name__}"
        )

    print("Generating reference/index.md...")
    with open(os.path.join(REFERENCE_DIR, "index.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(api_ref_lines))
        f.write("\n")

    print("\nGeneration complete!")
    print(f"Reference pages written to {REFERENCE_DIR}")
