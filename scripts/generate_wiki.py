...
def clean_type_str(t: Any) -> str:
    ...
    s = (
        s.replace("typing.", "")
        .replace("warera.resources.", "")
        .replace("warera.models.", "")
        .replace("warera._enums.", "")
    )
...
def format_type_with_link(t: str) -> str:
    def repl(m: re.Match[str]) -> str:
        ...
        if (
            ...
            and word not in ["Any", "None", "Optional"]
        ):
    ...
    if sig.parameters:
        ...
        for name, param in sig.parameters.items():
            if name == "self":
                continue
            ptype = (
                clean_type_str(param.annotation)
...
def run() -> None:
    ...
    print("\nGeneration complete!")
    print(
        f"Check the {WIKI_DIR} directory. You can now cd into it, and manually `git commit` and `git push`."
    )
